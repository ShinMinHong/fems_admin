 ...
 */
@Component
public class AppInsecureChannelProcessor extends InsecureChannelProcessor {
	...
	public void decide(FilterInvocation invocation, Collection<ConfigAttribute> config) throws IOException, ServletException {
		logger.debug("***** AppInsecureChannelProcessor decide  ==> invocation : {}, config : {}", invocation, config);
		...
		logger.debug("***** AppInsecureChannelProcessor decide  ==> forwardedProtocol : {}", forwardedProtocol);

		for(ConfigAttribute attribute : config) {
			logger.debug("***** AppInsecureChannelProcessor decide  ==> attribute : {}", attribute);
			if(supports(attribute)) {
				logger.debug("***** AppInsecureChannelProcessor decide supports ==> attribute : {}", attribute);
				if( null != forwardedProtocol ) {
					if("https".equals(forwardedProtocol)) {
						logger.debug("***** INSECURE 1 ==> {} : {}", X_FORWARDED_PROTO, forwardedProtocol);
						getEntryPoint().commence(invocation.getRequest(), invocation.getResponse());
					}
				} else {
					if(invocation.getHttpRequest().isSecure()) {
						logger.debug("***** INSECURE 2 ==> {} : {}", X_FORWARDED_PROTO, forwardedProtocol);
						getEntryPoint().commence(invocation.getRequest(), invocation.getResponse());
					}
				}
			}
