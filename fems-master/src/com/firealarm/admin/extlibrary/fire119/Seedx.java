package com.firealarm.admin.extlibrary.fire119;
...
public class Seedx {
	...
	/* 암호화 하기위한 key- Server와 Client가 모두 이 Key를 맞춰주어야함. */
	byte[] key = "winitech_winitech".getBytes();
	...
	public byte[] encrypt(byte[] sbuffer, byte szKey[]) {
		...
		byte[] pbDataBuffer = padding.addPadding(sbuffer, SeedBlockSize);
		byte[] encryptBytes = new byte[pbDataBuffer.length];

		int rt = pbDataBuffer.length / SeedBlockSize;
		for (int j = 0; j < rt; j++) {
			byte sSource[] = new byte[SeedBlockSize];
			byte sTarget[] = new byte[SeedBlockSize];

			System.arraycopy(pbDataBuffer, (j * SeedBlockSize), sSource, 0,
					SeedBlockSize);

			SeedEncrypt(sSource, sRoundKey, sTarget); // KISA에서 제공하는 함호화 함수
	 ...
	 * @return byte[] 암호화 된 데이터
	 */
	public byte[] encrypt(String pdData, byte szKey[], String charset)
			throws UnsupportedEncodingException {

		return encrypt(pdData.getBytes(charset), szKey);
	}

	/**
	 * 암호화 된 바이트 데이터를 받아서 복호화한다.
	 *
	...
	public byte[] decrypt(byte[] encryptBytes, byte[] szKey) {
		...
		for (int j = 0; j < rt; j++) {
			...
			SeedDecrypt(sSource, sRoundKey, sTarget); // KISA 에서 제공하는 복호화 함수
	 ...
	 * @return byte[] 복호화 된 바이트 데이터
	 */
	public String decrypt(byte[] encryptBytes, byte[] szKey,
	...
	/*
	 *  KISA 의 Seed 알고리즘을 사용한 복호화
	 */
	public String DecryptByte(String value) throws UnsupportedEncodingException{
		if(value == null || value.equals("")){
			return "";
		}

		// 복호화
		byte[] encryptbyte = Base64.decode(value,Base64.NO_WRAP);
		String decryptText = this.decrypt(encryptbyte, key, "UTF-8");

		return decryptText;
	}

	/*
	 * KISA 의 Seed 알고리즘을 사용한 암호화
	 */
	public String EncryptByte(String value) throws UnsupportedEncodingException {
		if(value == null || value.equals("")){
			return "";
		}
		// 암호화
		String encryptText = Base64.encodeToString(this.encrypt(value, key, "UTF-8"),Base64.NO_WRAP);
		return encryptText;
	}
