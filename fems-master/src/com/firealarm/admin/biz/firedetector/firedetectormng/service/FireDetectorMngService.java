package com.firealarm.admin.biz.firedetector.firedetectormng.service;
...
import com.firealarm.admin.biz.firedetector.firedetectormng.vo.FireDetectorExcelDataRO;
import com.firealarm.admin.biz.firedetector.firedetectormng.vo.FireDetectorExcelDataVO;
 ...
 */
@Service
public class FireDetectorMngService extends ServiceSupport {
	...
	/** 전체 목록 조회 */
	public List<FireDetectorMngVO> getListAll(Sort sort, SearchMap search) {
		return fireDetectorMngDAO.getListAll(sort, search);
	}

	/** 페이징 목록 조회 */
	public Page<FireDetectorMngVO> getList(Pageable pageable, SearchMap search) {
		return fireDetectorMngDAO.getList(pageable, search);
	}

	/** 파일 상세 */
	public DownloadFileVO getFileInfoByAttachedFileSeq(long attachedFileSeq) {
		return fireDetectorMngDAO.getFileInfoByAttachedFileSeq(attachedFileSeq);
	}

	/** 파일  존재유무 */
	public Boolean hasAttachedFileSeq(long attachedFileSeq) {
		return fireDetectorMngDAO.hasAttachedFileSeq(attachedFileSeq);
	}

	/** 상세 */
	public FireDetectorMngVO getByFireDetectionSeq(long fireDetectorSeq) {
		FireDetectorMngVO vo = fireDetectorMngDAO.getByFireDetectionSeq(fireDetectorSeq);
		List<FireDetectorNowStatusDT> fireDetectorNowStatus = fireDetectorMngDAO.getFireDetectorNowStatusDT(fireDetectorSeq);
		vo.setFireDetectorNowStatusDT(fireDetectorNowStatus);
		vo.setBoardFiles(fireDetectorMngDAO.getFileListByFireDetectorSeq(fireDetectorSeq));
		return vo;
	}

	/** 등록[화재감지기/화재감지기파일/화재감지기현재상태] */
	@Transactional
	public void insert(FireDetectorMngActiveVO vo) {
		// CTN 번호 중복 확인
		if(fireDetectorMngDAO.hasDuplicatedCtnNo(vo.getCtnNo())) {
			throw new AbleRuntimeException("CTN 번호가 중복되었습니다.");
		}

		//화재감지기 등록 , 화재 감지기 현재상태 초기화 한 후 등록
		long fireDetectorSeq = fireDetectorMngDAO.insert(vo);
		...
		if (null != uploadfiles) {
			...
			if (uplodedFiles.size() > 0) {
				fireDetectorMngDAO.insertFileInfoListToDB(uplodedFiles,fireDetectorSeq);
			}
		}
	}
	...
	/** 수정 */
	@Transactional
	public void update(FireDetectorMngActiveVO vo) {
		// 감지기 정보 수정
		fireDetectorMngDAO.update(vo);
		...
		if (null != uploadfiles) {
			...
			if (uplodedFiles.size() > 0) {
				// 파일 정보 DB Insert
				fireDetectorMngDAO.insertFileInfoListToDB(uplodedFiles, vo.getFireDetectorSeq());
			}
		}
	}
	...
	public void delete(long fireDetectorSeq) {
		//파일들 삭제
		List<Long> attachedFileSeqList =fireDetectorMngDAO.getAttachedFileSeqByFireDetectorSeq(fireDetectorSeq);
		...
		fireDetectorMngDAO.delete(fireDetectorSeq);
	}

	/** 파일 단건 삭제 */
	@Transactional
	public void deletefile(long attachedFileSeq) {
		String pathToDelete = fireDetectorMngDAO.getFilePathByAttachedFileSeq(attachedFileSeq);
		fileStorageManager.deleteUploadedFile(pathToDelete, false);
		fireDetectorMngDAO.deleteFileByAttachedFileSeq(attachedFileSeq);
	}

	/** 점포선택해제 */
	public void disconnectionWithStore(String ctnNo, long fireDetectorSeq) {

		if(fireDetectorMngDAO.hasDuplicatedCtnNo(ctnNo)) {
			// 화재감지기 점포정보 초기화
			fireDetectorMngDAO.disconnectionWithStore(ctnNo);
			// 현재 상태 로그 초기화
			FireDetectorNowStatusDT dt =FireDetectorNowStatusDT.builder()
					.fireDetectorSeq(fireDetectorSeq)
					.alarmFire(false)
	...
	public ExcelUploadRO insertExcelUpload(ExcelUploadVO uploadVO){
		...
		if (ro.getFailCount() == 0 && ro.getTotalCount() > 0 ){ //실패건 없을시  DB 등록
			for( Object dataOb : ro.getExcelResultVO() ){
				...
				FireDetectorMngActiveVO actionVO = new FireDetectorMngActiveVO();
				// 등록자 , 수정자 정보
				...
				if(fireDetectorMngDAO.hasDuplicatedCtnNo(actionVO.getCtnNo())){//  중복된 CTN 있을시 화재감지기 Update
					actionVO.setUpdAdminId(adminId);
					fireDetectorMngDAO.update(actionVO);
				}else{ // 중복된 CTN 없을시 화재감지기 신규등록
					actionVO.setFireDetectorAckValue("0"); // ACK VALUE 초기화 상태
					actionVO.setRegAdminId(adminId);
					// 화재수신기 정보 등록
					long fireDetectorSeq = fireDetectorMngDAO.insert(actionVO);
					// 화재수신기 현재상태 정보 등록
					FireDetectorNowStatusDT dt =FireDetectorNowStatusDT.builder()
							.fireDetectorSeq(fireDetectorSeq)
							.alarmFire(false)
	...
	public void preventsubmission(String ctnNo) {
		if(fireDetectorMngDAO.hasDuplicatedCtnNo(ctnNo)) {
			fireDetectorMngDAO.preventsubmission(ctnNo);
		}else {
