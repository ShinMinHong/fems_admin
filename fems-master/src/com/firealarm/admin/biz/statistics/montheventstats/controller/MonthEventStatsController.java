...
/**
 * 월별 화재 신호 통계 Controller
 * @author rodem4_pc1
 *
 */
@Controller
@PreAuthorize("hasAuthority('ROLE_STATISTICS_READ')")
@RequestMapping(value = "/statistics/montheventstats")
public class MonthEventStatsController extends MngAreaControllerSupport {
	...
	public String indexList(Model model,
		...
		return "statistics/montheventstats/montheventstats";
	}
	...
	public View excelDownload(
		...
		//데이터 목록 획득
		List<MonthEventStatsVO> list = monthEventStatsService.getMonthEventStatsListAll(null, search);

		//엑셀 출력 명령 준비
		AbleExcelCommand command = new AbleExcelCommand(list, MonthEventStatsVO.class, messageSource);

		command.setFilename("월별 화재 신호 통계 정보");
		command.setTitle("월별 화재 신호 통계 정보");
		command.setSheetName("월별 화재 신호 통계 정보");
