...
/**
 * 점포별 화재 신호 통계 Controller
 * @author rodem4_pc1
 *
 */
@Controller
@PreAuthorize("hasAuthority('ROLE_STATISTICS_READ')")
@RequestMapping(value = "/statistics/storeeventstats")
public class StoreEventStatsController extends MngAreaControllerSupport {
	...
	public String indexList(Model model,
		...
		return "statistics/storeeventstats/storeeventstats";
	}
	...
	public View excelDownload(
		...
		//데이터 목록 획득
		List<StoreEventStatsVO> list = storeEventStatsService.getStoreEventStatsListAll(null, search);

		//엑셀 출력 명령 준비
		AbleExcelCommand command = new AbleExcelCommand(list, StoreEventStatsVO.class, messageSource);

		command.setFilename("점포별 화재 신호 통계 정보");
		command.setTitle("점포별 화재 신호 통계 정보");
		command.setSheetName("점포별 화재 신호 통계 정보");
