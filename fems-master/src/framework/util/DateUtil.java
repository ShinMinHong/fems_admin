...
public class DateUtil {
     ...
     * @throws FmsInvalidFormatException 요청된 DteTime 문자열의 형식이 올바르지 않을 경우 발생
     */
     ...
     * @return Joda DateTime (throwException 이 false면 변환실패시 null 반환)
     * @throws FmsInvalidFormatException 요청된 DateTime 문자열의 형식이 올바르지 않은 경우 발생(throwException이 ture 일때만)
     */
    public static DateTime parseToDateTime(String dateString, Boolean throwException) {
        ...
        if(dateString.matches("^\\d{4}-\\d{2}-\\d{2}$")) {
            ...
            if(throwException) {
                throw new AbleRuntimeException("Invalid DateTime Format. ['" + dateString + "']");
            } else {
                return null;
            }
        }
