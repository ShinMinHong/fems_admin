...
function RootVM() {
	...
	//List API URL
	self.listURL = contextPath + "/statistics/areaeventstats/api/areaeventstatspage.json";
	//Details REST API URL
	self.detailsURL = contextPath + "/statistics/areaeventstats/api/details";
	...
	self.onGridEvent = function(type, data) {
		switch(type) {
			case "rowSelected" :
				$.ajax({
					url: $.format("{0}/{1}/{2}/{3}.json", self.detailsURL, data.marketSeq,rootVM.searchData().searchStartDate(),rootVM.searchData().searchEndDate()),
					type: "GET",
	...
	self.excelDownload = function(){
		...
		var url = contextPath + "/statistics/areaeventstats/excel?";
