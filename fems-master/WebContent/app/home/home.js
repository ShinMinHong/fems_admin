...
function gridDemonRegDateFormat(dataItem){
	//return !s.isBlank(dataItem.demonRegDate) ? moment(dataItem.demonRegDate).format('YYYY-MM-DD<br/>HH:mm:ss') : "";
	return !s.isBlank(dataItem.demonRegDate) ? moment(dataItem.demonRegDate).format('YYYY-MM-DD HH:mm:ss') : "";
}

function HomeViewModel() {
	var self = this;
	self.clazz = "HomeViewModel";
	...
	self.searchMarketEnable = ko.observable(self.mapInitConf.mapUserType == 'MNG_AREA_USER');
	self.searchMarketSeq = ko.observable(self.mapInitConf.mapUserType == 'MARKET_USER' ?  pageParam.MAP_INIT_MARKET_SEQ : null);
	...
	self.getCurrentStatusList = function(){
		$.ajax({
			url: $.format("{0}/{1}.json", contextPath + "/home/api/getstatuslistinevent", pageParam.MNG_AREA_SEQ),
			type: "GET",
