...
(function() {
	...
	var VMExtender = function() {
		...
		extender.extendGridVM = function(targetVM) {
			...
			/**
			 *  코드맵으로 부터 해당 value 값을 받아 text 값으로 변환 
			 *   사용방법 ==> template: self.gridColumnTemplate.getValueFromCodeMap('roleGroupCode', [{"text":"본부관리자","value":"HQ_ADMIN"}...]) return- 본부관리자
			 */
			self.gridColumnTemplate.getTextFromCodeMap= function(field, codeMap, nodatamsg) {
				var closure = {
					field: field,
					codeMap: codeMap,
					nodatamsg: nodatamsg || ""
				...
				var template = function(dataItem) {
					var textValue = dataItem[closure.field];
						if(textValue){							
							var filterdValue = _.filter(closure.codeMap, function(row){return row.value==textValue});
							return (filterdValue && filterdValue.length ) ? filterdValue[0].text : closure.nodatamsg; 
						}else{
							return closure.nodatamsg;
						}
				};
			...
			/**
			 *   boolean 형태의 filed 값을 trueMsg false ,falseMsg 값으로 변환  
			 *   사용방법 ==> ex) 	{title: "사용여부", width:70, ezAlign: 'center' , template: self.gridColumnTemplate.TFCodeMap('useYn','사용','미사용') },
			 */
			self.gridColumnTemplate.TFCodeMap= function(field, trueMsg, falseMsg) {
				var closure = {
					field: field,
				...
				var template = function(dataItem) {
					...
					if(typeof textValue == "string"){
						return textValue.toLowerCase() == 'true' ? closure.trueMsg : closure.falseMsg; 
					}
					return 	textValue ? closure.trueMsg : closure.falseMsg;
				};
		...
		extender.extendPopupVM = function(targetVM) {
			...
			if(s.isBlank(self["popupId"])) {
				console.log("VM에 popupId를 정의해주세요.");
			}
			if(s.isBlank(self["popupOptions"])) {
		 ...
		 * - close
		 */
		extender.extendMapIframePopupVM = function(targetVM) {
			if( ezUtil.ezInitialized === false ){
				//ezUtil Initialize가 진행되지 않은 경우 Initialize 실행
				ezUtil.ezInitialize();
			}
			var self = targetVM;
			if(self["_extendMapIframePopupVM_processed"] === true) {
				return; //중복확장 방지
			}
			if(_.isBlank(self["popupId"])) {
				console.log("VM에 popupId를 정의해주세요.");
			...
			//확장되었다는 플래그 처리
			self._extendMapIframePopupVM_processed = true;
		}; //extendPopupVM
