...
function AdminMngVM() {
	...
	//Base URL
	self.baseURL = contextPath + "/areasystem/adminmng";
	//List API URL
	...
	// 코드맵 (관리자 구분 , 지역 , 시장)
	var userGradeCodeMap =  ezUtil.orderByAuthorGroupSelectOptions(pageParam.USER_GRADE_CODE_MAP);
	self.appUserGradeSelectOptions = _.filter(userGradeCodeMap, function(x) { return x.value != 'HQ_ADMIN' });
	self.areaSelectOptions =  ezUtil.orderBySelectOptionsText( pageParam.MNG_AREA_CODE_MAP);
	// SelectOption의 value에 해당하는 text추출
	self.mngAreaName = ezUtil.findByValue(pageParam.MNG_AREA_CODE_MAP,pageParam.MNG_AREA_SEQ);
	self.marketSelectOptions =pageParam.MARKET_CODE_MAP;

	//Search VM
	self.searchData = {
		searchRolegroupCode:"", // 관리자 구분
	...
	self.gridColumns = [
		...
		{ title: "지역명", width: 150, ezAlign: 'left', template: self.gridColumnTemplate.getTextFromCodeMap('mngAreaSeq',pageParam.MNG_AREA_CODE_MAP)},
		{ title: "시장명", width: 150, ezAlign: 'left', template: self.gridColumnTemplate.getTextFromCodeMap('marketSeq',pageParam.MARKET_CODE_MAP)},
...
function DataDetailsVM(detailsURL, onDetailsChanged) {
	...
	self.validationRules = {
		common: {
			rolegroupCode:{required: true}, // 관리자 구분
	...
	var setData = function(data) {
		...
		if ( dataCloned != null ){
			...
			// 관리자 권한 변경시 필수입력사항 변경 및 validation변경( 관제지역,시장선택)
			dataCloned.rolegroupCode.subscribe(function(newValue){
				switch(newValue){
					case 'MARKET_ADMIN':
						// 필수 사항 표시
						self.isRequiredForArea("*");
						self.isRequiredForMarket("*");
						self.data().mngAreaSeq(pageParam.MNG_AREA_SEQ);
						// 관리자 구분에 따른 validation
						self.validationRules.customizedValidation = {mngAreaSeq:{required: true}, marketSeq :{required: true}};
						break;
					case 'AREA_ADMIN':
						//초기화
						self.data().mngAreaSeq(pageParam.MNG_AREA_SEQ);
						self.data().marketSeq(null);
						// 필수사항
						self.isRequiredForArea("*");
						self.isRequiredForMarket("");
						//validation
						self.validationRules.customizedValidation = {mngAreaSeq:{required: true}, marketSeq :{required: false}};
						break;
					case 'HQ_ADMIN':
					default:
						//초기화
						self.data().marketSeq(null);
						self.data().mngAreaSeq(null);
						// 필수사항
						self.isRequiredForArea("");
						self.isRequiredForMarket("");
						//validation
						self.validationRules.customizedValidation = {mngAreaSeq:{required: false}, marketSeq :{required: false}};
						break;
				}
			});
		}
