...
(function() {
	...
	ko.bindingHandlers.datePicker = {
		'init': function(element, valueAccessor) {
			...
			var date = ko.isObservable(value) ? moment(ko.unwrap(value)).format('YYYY-MM-DD') : moment(value).format('YYYY-MM-DD');
			if(_.isNull(value())){
	    		date = "0000-00-00";
	    	}
			var defaultOption = {
					dateIcon : false,
					format : 'yyyy-MM-dd',
					footer : "#= moment(serverTime).lang('ko').format('YYYY년 MM월 DD일 dddd') #"
				};
	    	var defaultMaskedOption = {
					mask : "0000-00-00",
					promptChar : "_"
				};
	    	var regExp = /^\d{4}-\d{2}-\d{2}$/;

	    	value = _.defaults(value||{}, defaultOption);
	    	value.maskedOption = _.defaults(value['maskedOption']||{}, defaultMaskedOption);
	    	if(regExp.test(date)){
	    		date = ko.observable(moment(date).format('YYYY-MM-DD'));
	    		$(element).kendoMaskedTextBox(_.defaults(_.clone(value.maskedOption), {
		    		value: date(),
					change: function() {
					    console.log("kendoMaskedTextBox changed:" + this.value());
					    date(this.value());
					}
				}));
	    	}
	    ...
	    'update': function (element, valueAccessor) {
	    	...
	    	if(regExp.test(value()) && !moment(value()).isValid()){
	...
	ko.bindingHandlers.rangeDatePicker = {
		'init': function(element, valueAccessor) {
			...
			startElement.kendoDatePicker({
	    		culture: "ko-KR",
	    		format: "yyyy-MM-dd",
			}).data("kendoDatePicker");
			endElement.kendoDatePicker({
	    		culture: "ko-KR",
	    		format: "yyyy-MM-dd",
			}).data("kendoDatePicker");

			var startDate = ko.isObservable(option.value) ? moment(ko.unwrap(option.value)).format('YYYY-MM-DD') : moment(option.value).format('YYYY-MM-DD');
	    	var endDate = ko.isObservable(option.endValue) ? moment(ko.unwrap(option.endValue)).format('YYYY-MM-DD') : moment(option.endValue).format('YYYY-MM-DD');

	    	if(_.isNull(option.value() && option.endValue())){
	    	...
	    	option.maskedOption = _.defaults(option['maskedOption']||{}, defaultMaskedOption);

	    	if(regExp.test(startDate) && regExp.test(endDate)){
	    		startDate = ko.observable(moment(startDate).format('YYYY-MM-DD'));
		    	startElement.kendoMaskedTextBox(_.defaults(_.clone(option.maskedOption), {
		    		value: startDate(),
					change: function() {
					    console.log("kendoMaskedTextBox changed:" + this.value());
		    	...
		    	endDate = ko.observable(moment(endDate).format('YYYY-MM-DD'));
		    	endElement.kendoMaskedTextBox(_.defaults(_.clone(option.maskedOption), {
		    		value: endDate(),
					change: function() {
					    console.log("kendoMaskedTextBox changed:" + this.value());
					    endDate(this.value());
	    ...
	    'update': function (element, valueAccessor) {
	    	...
	    	if(regExp.test(option.value()) && !moment(option.value()).isValid()){
	    		$(element).data("kendoDatePicker").value(null);
	    	}
	    	if(regExp.test(option.endValue()) && !moment(option.endValue()).isValid()){
	    		_.defer(function(){
	    			$("#"+ option.endElementId).data("kendoDatePicker").value(null);
	    		});
	    	}
	    	...
	    	if(regExp.test(startDate) && regExp.test(endDate)){
		    	...
		    	self.preventReversalOfKoDateRange(startDate, endDate);
	...
	self.preventReversalOfKoDateRange = function(startDateKoObservable, endDateKoObservable, dateFormat) {
		...
		//Date Range - Start/End 역전 제한
			if(!self.validateDateISOFormat(startDateKoObservable())) {
				return;
			}
			if(!self.validateDateISOFormat(endDateKoObservable())) {
				return;
			}
			var start = moment(startDateKoObservable(), _dateFormat);
			var end = moment(endDateKoObservable(), _dateFormat);
			if(end.diff(start) < 0) {
				return endDateKoObservable(startDateKoObservable());
			}
			if(start.diff(end) > 0) {
				return startDateKoObservable(endDateKoObservable());
			}
	};

	/**
	 * 주어진 날짜 문자열이 올바른 Date 형태인지 확인 (기본형태: YYYY-MM-DD)
	 */
	self.validateDateISOFormat = function(dateISOString, dateFormat) {
		if(!dateISOString.match(/^\d{4}-\d{2}-\d{2}$/)) {
			return false;
		}
		var _dateFormat = dateFormat || "YYYY-MM-DD";
		if(!moment(dateISOString, _dateFormat).isValid()) {
			return false;
		}
		return true;
	};
