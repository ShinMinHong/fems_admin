...
$(function() {
	...
	*/
	...
	(function ($) {
		...
		*/
		...
		$.extend($.validator.messages, {
			...
			float2: "__FIELD_NAME__ 항목은 정수나 소수점 둘째자리까지 입력가능합니다.\n(정수는 최대 8자리까지 입력 가능 합니다.)",
			...
			stringDate : $.validator.format("__FIELD_NAME__ 항목은  YYYYMMDD 형식으로 입력하세요."),
			//jquery validate additional
			...
			listNotEmpty: "__FIELD_NAME__ 항목을 선택하여 주세요."
		});
	}(jQuery));
	...
	jQuery.validator.addMethod("stringDate", function(value, element) {
		// YYYYMMDD ( 20세기 21세기)
		return this.optional(element) || /^((1[9]|2[0-1])(\d{2})(0[1-9]|1[0-2])([0-3])(\d{1}))$/.test(value);
	}, jQuery.validator.messages.stringDate);
	...
	jQuery.validator.addMethod("listNotEmpty", function(value, element, param){
		try{
			var selectedText = $("#"+element.id+" option:selected").text();
			if(selectedText === param){
				return false;
			}
			return true;
		} catch(ex) {
			//선택값을 찾지 못한 경우는 true로 처리. 화면에서 block되는 현상을 차단하기 위해.
			return true;
		}
	 }, jQuery.validator.messages.listNotEmpty);
